...
sys.path.insert(0, r"C:\tmp\roformer2")
from models.bs_roformer.bs_roformer import BSRoformer, pack_one, unpack_one

with open("config_bs_roformer.yaml", "r") as f:
...
ckpt = torch.load("model_bs_roformer_ep_317_sdr_12.9755.ckpt", map_location="cpu", weights_only=False)
...
chunk_size = cfg["inference"]["dim_t"] * cfg["model"]["stft_hop_length"]
...
def export_forward(self, raw_audio):
    ...
    m_re, m_im = mask[..., 0], mask[..., 1]
    out_re = a_re * m_re - a_im * m_im
    out_im = a_re * m_im + a_im * m_re
    masked = torch.stack([out_re, out_im], dim=-1)
...
orig_module = importlib.import_module("models.bs_roformer.bs_roformer")
orig_model = orig_module.BSRoformer(**model_cfg)
